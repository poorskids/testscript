    ...
    return {
        Disconnect = function()
            for i,v in ipairs(self._binds) do
                if v == fn then table.remove(self._binds, i) break end
            end
        end
    }
end
...
function Signal:Wait()
    ...
    self:Connect(function(...) coroutine.resume(thread, ...) end)
...
on_gui_fade:Connect(function()
    ...
    local tween = TS:Create(ImageLabel, TweenInfo.new(0.25), {ImageTransparency = 1})
    tween:Play()
...
local function fade_in()
    local TS = game:GetService("TweenService")
    TS:Create(Frame, TweenInfo.new(0.3), {BackgroundTransparency = 0.5}):Play()
    TS:Create(blur, TweenInfo.new(0.3), {Size = 24}):Play()
    TS:Create(ImageLabel, TweenInfo.new(0.3), {ImageTransparency = 0}):Play()
end
...
Mouse.KeyDown:Connect(function(Key)
    ...
    local Keybind7 = Arceus.Silent.TriggerBotKey:lower()
    if Key == Keybind7 and Arceus.Silent.UseTriggerBotKeybind then
        Arceus.Silent.TriggerBot = not Arceus.Silent.TriggerBot
        if Arceus.Both.SendNotification then
            game.StarterGui:SetCore("SendNotification",{
                Title = "Noah Private",
                Text = "TriggerBot: " .. tostring(Arceus.Silent.TriggerBot),
                Icon = "rbxassetid://16897920243",
                Duration = 1
            })
        end
...
-- // Right Mouse Button Input Events
...
-- // Silent Aim Misc (WITH FIXED TRIGGERBOT - REQUIRES RIGHT MOUSE BUTTON AND SPECIFIC WEAPONS)
Script.Functions.SilentMisc = LPH_NO_VIRTUALIZE(function()
    if Arceus.Silent.Enabled then
        if Arceus.Silent.UseAirPart and Script.Functions.Alive(SilentTarget) then
        ...
        -- FIXED TRIGGERBOT: Only shoots when right mouse button is held, we have a target, specific weapon, and target is within FOV
        if Arceus.Silent.TriggerBot and Script.Functions.Alive(SilentTarget) and RightMouseButtonHeld then
            -- Check if we have a tool equipped
            if Client.Character and Client.Character:FindFirstChildWhichIsA("Tool") then
                local tool = Client.Character:FindFirstChildWhichIsA("Tool")
                -- Get the actual gun name from the tool name
                local gunName = Script.Functions.GetCurrentWeaponName()
                
                -- Check if it's actually a gun and one of the allowed weapons
                if tool and gunName then
                    -- Only allow TriggerBot for Revolver, Double Barrel, or Tactical Shotgun
                    ...
                    -- Check if current weapon is in allowed list
                    if allowedWeapons[gunName] then
                        -- Check if target is within FOV
                        local Distance = Script.Functions.GetMagnitudeFromMouse(SilentTarget.Character.HumanoidRootPart)
                        if Distance <= Arceus.SilentFov.Radius * 3 then -- Multiply by 3 as in your UpdateFOV function
                            mouse1click()
                        end
                    end
                end
